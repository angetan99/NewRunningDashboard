...
import { 
  ...
  getAveragePace,
  updateUserProfile,
  getAgeGradingStats  // Make sure this is here!
} from './database.js';
...
// Home page - Public Family Dashboard
app.get('/', async (req, res) => {
  const users = getAllUsers();
  
  if (users.length === 0) {
    // No users yet - show welcome page
    return res.send(`
      <!DOCTYPE html>
      <html>
      <head>
        <title>Zhou Family Running Challenge</title>
        <style>
          body { 
          ...
          .connect-button { margin: 30px 0; }
          .connect-button img { height: 48px; cursor: pointer; }
          .powered-by { margin-top: 40px; font-size: 12px; color: #666; }
        </style>
      </head>
      <body>
        <div class="hero">
          <h1>🏃 Zhou Family Running Challenge</h1>
          <p class="subtitle">Year-long competition with age-graded performance tracking</p>
          
          <div class="connect-button">
            <a href="/auth/strava">
              <img src="/btn_strava.svg" alt="Connect with Strava" />
            </a>
          </div>
  ...
  // FAMILY DASHBOARD - Public page showing everyone
  try {
    const today = new Date();
    const thirtyDaysAgo = new Date(today);
    thirtyDaysAgo.setDate(thirtyDaysAgo.getDate() - 30);
    
    // Gather stats for all users (with live Strava data for age grading)
    const familyStats = [];
    ...
    for (const user of users) {
      ...
      if (user.age && user.sex && user.baseline_mile_pace && user.access_token) {
        try {
          ...
          if (recentRuns.length > 0) {
            ...
            improvement = calculateImprovement(avgPace, user.baseline_mile_pace, user.age, user.sex) || 0;
          }
        } catch (error) {
      ...
      familyStats.push({
        ...
        todayComplete: todayProgress?.status === 'completed',
        improvement,
        age: user.age,
        sex: user.sex
      });
    ...
    // Sort by total days run (for ranking)
    ...
    // Prepare chart data
    const chartLabels = rankedStats.map(s => s.name.split(' ')[0]);
    const chartDaysData = rankedStats.map(s => s.totalDaysRun);
    const chartMilesData = rankedStats.map(s => s.totalMiles.toFixed(1));
    const chartStreakData = rankedStats.map(s => s.currentStreak);
    const chartImprovementData = rankedStats.map(s => s.improvement.toFixed(1));
    ...
    const html = `
      ...
      <body>
        <div class="container">
          <div class="header">
            <h1>🏃 Zhou Family Running Challenge</h1>
            <div class="subtitle">Year-Long Competition · Age-Graded Performance</div>
            <div class="date">📅 ${today.toLocaleDateString('en-US', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })}</div>
          </div>
          
          <div class="quick-stats">
            <div class="quick-stat">
              <div class="number">${familyStats.filter(s => !s.eliminated).length}</div>
              <div class="label">Active Runners</div>
            </div>
            <div class="quick-stat">
              <div class="number">${familyStats.reduce((sum, s) => sum + s.totalDaysRun, 0)}</div>
              <div class="label">Total Days Run</div>
            </div>
            <div class="quick-stat">
              <div class="number">${familyStats.reduce((sum, s) => sum + s.totalMiles, 0).toFixed(0)}</div>
              <div class="label">Total Miles</div>
            </div>
            <div class="quick-stat">
              <div class="number">${familyStats.filter(s => s.todayComplete).length}/${familyStats.length}</div>
          ...
          <div class="dashboard-grid">
            <div class="card">
              ...
              ${rankedStats.map((stat, index) => {
                ...
                return `
                  <div class="leaderboard-item ${stat.eliminated ? 'eliminated' : ''} ${stat.status === 'at_risk' ? 'at-risk' : ''}">
            ...
            <div class="card">
              ...
              <p style="text-align: center; color: #666; font-size: 12px; margin-top: 10px;">
                Pace improvement adjusted for age & sex vs. baseline
              </p>
            </div>
          </div>
          ...
          <div class="actions">
            ${req.session.userId 
              ...
              : '<a href="/auth/strava" class="btn">🔗 Connect Strava</a>'
            }
        ...
        <script>
          ...
          new Chart(daysCtx, {
            ...
            data: {
              ...
              datasets: [{
                ...
                data: ${JSON.stringify(chartDaysData)},
          ...
          new Chart(milesCtx, {
            ...
            data: {
              ...
              datasets: [{
                label: 'Total Miles',
                data: ${JSON.stringify(chartMilesData)},
                backgroundColor: 'rgba(252, 82, 0, 0.8)',
                borderColor: 'rgba(252, 82, 0, 1)',
                borderWidth: 2
              }]
          ...
          new Chart(improvementCtx, {
            ...
            data: {
              ...
              datasets: [{
                label: 'Improvement %',
                data: ${JSON.stringify(chartImprovementData)},
                backgroundColor: ${JSON.stringify(chartImprovementData.map(val => 
                  parseFloat(val) > 0 ? 'rgba(40, 167, 69, 0.8)' : 'rgba(220, 53, 69, 0.8)'
                ))},
                borderColor: ${JSON.stringify(chartImprovementData.map(val => 
                  parseFloat(val) > 0 ? 'rgba(40, 167, 69, 1)' : 'rgba(220, 53, 69, 1)'
                ))},
                borderWidth: 2
              }]
            },
